...
Dola 代理节点使用 Dola 首页作为延迟与可用性探测目标；实际提交前还会通过选中出口检查最终地址，地区限制节点立即进入冷却。Mihomo 并发任务通过引用计数租约固定当前出口，活动上下文释放前不会切换节点，避免同一任务运行过程中公网 IP 变化。HTTP、HTTPS 与 SOCKS 出口均执行相同探测。
...
Dola 返回“当前服务访问频繁”时，Worker 保持当前页面，并在 15 秒内每 500 毫秒检查一次滑块和登录状态；可见元素出现独立“登录”文本时立即将账号标记为登录异常并切换账号。只有整个观察窗口内页面内容、地址和风险状态均无变化时才刷新一次并重新检查，避免刷新重置账号验证状态。

用户端不会展示 Dola 的“超过 10 秒暂不支持”原始原因，任务列表、单任务查询和页面兜底提示统一显示“生成接口繁忙请稍后重试！”；管理员端继续保留原始诊断。
...
节点订阅测速会按后台“高延迟阈值”过滤高延迟和不可用节点。自动模式会保存阈值内延迟最低的节点并在节点列表中显示选中状态，Worker 的实际任务选路使用同一阈值。

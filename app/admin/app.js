...
const els = {
  ...
  membershipDetailsModal: document.getElementById("membershipDetailsModal"),
  closeMembershipDetailsModal: document.getElementById("closeMembershipDetailsModal"),
  cancelMembershipDetailsModal: document.getElementById("cancelMembershipDetailsModal"),
  membershipDetailsList: document.getElementById("membershipDetailsList"),
  ...
  billingPriorityControl: document.getElementById("billingPriorityControl"),
  billingPriorityState: document.getElementById("billingPriorityState"),
  ...
  promptManagerPanel: document.getElementById("promptManagerPanel"),
  ...
  retrySelectedBatchTasks: document.getElementById("retrySelectedBatchTasks"),
  retryAllFailedBatchTasks: document.getElementById("retryAllFailedBatchTasks"),
  ...
  batchPrevPage: document.getElementById("batchPrevPage"),
  batchNextPage: document.getElementById("batchNextPage"),
  batchPageState: document.getElementById("batchPageState"),
  batchAutoModal: document.getElementById("batchAutoModal"),
  ...
  qianwenSubmitInterval: document.getElementById("qianwenSubmitInterval"),
  taskRetryLimit: document.getElementById("taskRetryLimit"),
  doubaoSubmitRetryLimit: document.getElementById("doubaoSubmitRetryLimit"),
  ...
  membershipTaskDiscount: document.getElementById("membershipTaskDiscount"),
...
function renderPrompts() {
  ...
  els.promptManagerPanel?.classList.toggle("is-empty", state.prompts.length === 0);
...
function clientSafeText(value, task = {}) {
  ...
  if (/reference image upload timed out|prepare_upload timed out/i.test(text)) return terminal ? "参考图上传超时，请重试！" : "参考图上传超时，正在重试！";
...
function startClientLoginTransition() {
  ...
  const delay = window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 0 : 1750;
...
function applyAccessScope(data = {}) {
  ...
  const priorityInput = els.billingPriorityControl?.querySelector(`input[value="${state.billingPriority}"]`);
  if (priorityInput) priorityInput.checked = true;
  if (els.billingPriorityState) els.billingPriorityState.textContent = state.billingPriority === "points_first" ? "积分优先" : "视频额度优先";
...
function updateMembershipPurchaseButtons() {
  ...
  const highestPackage = state.memberships.reduce((highest, item) => Number(item.points_cost || 0) > Number(highest?.points_cost || 0) ? item : highest, null);
  const highestPackageId = String(highestPackage?.id || "");
  const highestPurchased = highestPackageId && purchasedIds.has(highestPackageId);
  els.membershipList?.querySelectorAll("[data-membership-id]").forEach((button) => {
    ...
    button.disabled = purchased || highestPurchased;
    button.textContent = purchased ? "有效期内已购买" : highestPurchased ? "最高级会员生效中" : "积分购买";
  });
}

function membershipDurationLabel(seconds) {
...
async function loadClientProfile() {
  ...
  state.membershipHoldings = Array.isArray(data.memberships) ? data.memberships : [];
  ...
  els.membershipDetailsButton?.classList.toggle("hidden", state.membershipHoldings.length === 0);
...
function renderProxyNodes() {
  ...
  const apiMode = state.proxySource === "api";
...
function renderApiProxyNodes() {
  const pool = state.proxyApiPool || {};
  const nodes = Array.isArray(state.proxyNodes) ? state.proxyNodes : [];
  const active = Math.max(0, Number(pool.active || 0));
  const capacity = Math.max(0, Number(pool.capacity || 0));
  if (els.proxyNodeCount) {
    els.proxyNodeCount.textContent = `${nodes.length} 个已提取 IP · 使用中 ${active}${capacity ? ` / ${capacity}` : ""}`;
  }
  if (!nodes.length) {
    const error = String(pool.last_error || "");
    els.proxyNodeGrid.innerHTML = `<div class="empty-state">${error ? `暂无可用 API 代理：${escapeHtml(error)}` : "尚未提取代理。生成任务开始后，已分配的代理会显示在这里。"}</div>`;
    return;
  }
  els.proxyNodeGrid.innerHTML = nodes.map((node) => {
    const activeCount = Math.max(0, Number(node.active || 0));
    const leases = Math.max(0, Number(node.total_leases || 0));
    const stateText = activeCount ? `使用中 · ${activeCount} 个任务` : "空闲";
    return `<article class="proxy-node-card api-proxy-node-card${activeCount ? " selected" : ""}">
      <span class="proxy-node-name">${escapeHtml(node.host_port || "-")}</span>
      <strong class="api-proxy-state${activeCount ? " good" : ""}">${escapeHtml(stateText)}</strong>
      <span class="proxy-node-country">累计分配 ${escapeHtml(leases)} 次 · 单 IP 同时 1 个任务</span>
    </article>`;
  }).join("");
}
...
async function loadProxyNodes(refresh = false, refreshSubscription = refresh) {
  ...
  try {
    ...
    state.proxyApiPool = data.pool && typeof data.pool === "object" ? data.pool : null;
...
async function loadRuntimeConfig() {
  ...
  const interval = Math.max(3, Math.min(30, Number(data.dola_global_submit_interval_seconds || 8)));
  const qianwenInterval = Math.max(1, Math.min(30, Number(data.qianwen_submit_interval_seconds || 5)));
  const retryLimit = Math.max(0, Math.min(10, Number.parseInt(data.task_retry_limit ?? 2, 10)));
  const doubaoRetryLimit = Math.max(0, Math.min(10, Number.parseInt(data.doubao_submit_retry_limit ?? 2, 10)));
  ...
  if (els.qianwenSubmitInterval) els.qianwenSubmitInterval.value = String(qianwenInterval);
  if (els.taskRetryLimit) els.taskRetryLimit.value = String(retryLimit);
  if (els.doubaoSubmitRetryLimit) els.doubaoSubmitRetryLimit.value = String(doubaoRetryLimit);
  if (els.batchHistoryRetentionDays) els.batchHistoryRetentionDays.value = String(retentionDays);
  if (els.runtimeConfigState) els.runtimeConfigState.textContent = `Dola ${interval} 秒/条 · 千问 ${qianwenInterval} 秒/条 · 换号 ${retryLimit} 次 · 豆包单号重发 ${doubaoRetryLimit} 次 · ${retentionDays} 天`;
  return data;
}
...
async function saveRuntimeConfig(event) {
  ...
  const qianwenInterval = Math.round(Number(els.qianwenSubmitInterval?.value || 0) * 10) / 10;
  const retryLimit = Number.parseInt(els.taskRetryLimit?.value || "", 10);
  const doubaoRetryLimit = Number.parseInt(els.doubaoSubmitRetryLimit?.value || "", 10);
  const retentionDays = Number.parseInt(els.batchHistoryRetentionDays?.value || "", 10);
  if (!Number.isFinite(interval) || interval < 3 || interval > 30) return toast("全局提交间隔需为 3 - 30 秒", "error");
  if (!Number.isFinite(qianwenInterval) || qianwenInterval < 1 || qianwenInterval > 30) return toast("千问提交间隔需为 1 - 30 秒", "error");
  if (!Number.isInteger(retryLimit) || retryLimit < 0 || retryLimit > 10) return toast("任务重试次数需为 0 - 10 次", "error");
  if (!Number.isInteger(doubaoRetryLimit) || doubaoRetryLimit < 0 || doubaoRetryLimit > 10) return toast("豆包单号重发次数需为 0 - 10 次", "error");
  ...
  try {
    const data = await apiFetch("/config/runtime", { method: "POST", body: { dola_global_submit_interval_seconds: interval, qianwen_submit_interval_seconds: qianwenInterval, task_retry_limit: retryLimit, doubao_submit_retry_limit: doubaoRetryLimit, batch_history_retention_days: retentionDays } });
    els.dolaSubmitInterval.value = String(data.dola_global_submit_interval_seconds);
    if (els.qianwenSubmitInterval) els.qianwenSubmitInterval.value = String(data.qianwen_submit_interval_seconds);
    if (els.taskRetryLimit) els.taskRetryLimit.value = String(data.task_retry_limit);
    if (els.doubaoSubmitRetryLimit) els.doubaoSubmitRetryLimit.value = String(data.doubao_submit_retry_limit);
    if (els.batchHistoryRetentionDays) els.batchHistoryRetentionDays.value = String(data.batch_history_retention_days);
    if (els.runtimeConfigState) els.runtimeConfigState.textContent = `Dola ${data.dola_global_submit_interval_seconds} 秒/条 · 千问 ${data.qianwen_submit_interval_seconds} 秒/条 · 换号 ${data.task_retry_limit} 次 · 豆包单号重发 ${data.doubao_submit_retry_limit} 次 · ${data.batch_history_retention_days} 天`;
    toast("运行与保留设置已生效");
  } catch (error) {
    toast(`保存失败：${error.message}`, "error");
  } finally {
...
async function refreshTasks(options = {}) {
  ...
  els.taskTableBody?.setAttribute("aria-busy", "true");
  try {
...
function getTaskStatus(task) {
  ...
  const statusReason = clientSafeText(task.status_reason || "", task);
  const diagnostic = portal === "client" ? "" : String(task.infrastructure_error || "");
  ...
  if (rawStatus === "pending" && retrying) {
    ...
    return { state: "running", label: "重试已入队", className: "running", text: retryProgress, url: "" };
  }
...
async function deleteTask(id) {
  ...
  try {
    ...
    if (data.cancelable === false) {
      toast(data.detail || "已提交生成，无法取消", "error");
      return;
    }
    if (isRunning) {
...
function createBatchImageEntries(files) {
  return Array.from(files || []).slice(0, 9).map((file) => ({ file, previewUrl: URL.createObjectURL(file) }));
}

function releaseBatchImageEntries(entries) {
  (entries || []).forEach((entry) => {
    if (entry?.previewUrl) URL.revokeObjectURL(entry.previewUrl);
  });
}

function appendBatchImageFiles(item, files) {
  const merged = [...(item.images || []), ...createBatchImageEntries(files)];
  item.images = merged.slice(0, 9);
  releaseBatchImageEntries(merged.slice(9));
}

function clearBatchReferenceImages() {
...
function batchSelectableEntries() {
  const persistent = Boolean(state.batchJobId);
  return state.batchPrompts.filter((item) => {
    if (!item.prompt.trim() || !batchItemMatchesStatus(item)) return false;
    return persistent
      ? String(item.status || "") === "failed" && Boolean(String(item.taskId || ""))
      : !batchItemIsCreated(item);
  });
}
...
function batchSelectedEntries() {
  return state.batchPrompts
    ...
    .filter(({ item }) => item.selected && batchSelectableEntries().includes(item));
}
...
function renderBatchPrompts() {
  ...
  const filteredEntries = state.batchPrompts
    .map((item, index) => ({ item, index }))
    .filter(({ item }) => batchItemMatchesStatus(item));
  const previousPage = Number(els.batchPromptList.dataset.page || 0);
  const previousScrollTop = els.batchPromptList.scrollTop;
  const totalPages = Math.max(1, Math.ceil(filteredEntries.length / state.batchPageSize));
  state.batchPage = Math.max(1, Math.min(totalPages, state.batchPage));
  const start = (state.batchPage - 1) * state.batchPageSize;
  const visiblePrompts = filteredEntries.slice(start, start + state.batchPageSize);
  if (!state.batchPrompts.length) {
    els.batchPromptList.innerHTML = '<div class="empty-state">导入表格后点击自动解析</div>';
  } else {
    els.batchPromptList.innerHTML = visiblePrompts.map(({ item, index }) => {
      const selectableRow = batchSelectableEntries().includes(item);
      const editable = !state.batchJobId && !state.batchSubmitting && !batchItemIsCreated(item);
      ...
      <article class="batch-prompt-row${item.status ? ` is-${escapeHtml(item.status)}` : ""}" data-batch-prompt-row="${index}">
        <input type="checkbox" data-batch-prompt-select="${index}" ${item.selected ? "checked" : ""} ${selectableRow && !state.batchRetrying ? "" : "disabled"} aria-label="选择表格第 ${escapeHtml(item.row)} 行" />
        <span class="batch-prompt-index">第 ${escapeHtml(item.row)} 行</span>
        <textarea data-batch-prompt-text="${index}" maxlength="4000" ${editable ? "" : "readonly"}>${escapeHtml(item.prompt)}</textarea>
        <div class="batch-prompt-reference"><div class="batch-row-thumbs">${previews}</div><button class="text-button" type="button" data-batch-image-index="${index}" ${editable ? "" : "disabled"}>参考图</button><span title="${escapeHtml(displayImages.map((entry) => entry.file.name).join("、"))}">${escapeHtml(referenceText)}</span>${images.length ? `<button class="batch-image-clear" type="button" data-batch-image-clear="${index}" aria-label="清除第 ${escapeHtml(item.row)} 行单独参考图" ${editable ? "" : "disabled"}>×</button>` : ""}</div>
        <div class="batch-prompt-result"><span class="batch-prompt-status">${escapeHtml(batchItemStatusText(item))}</span>${videoActions}</div>
        <button class="batch-prompt-delete" type="button" data-delete-batch-prompt="${index}" aria-label="删除表格第 ${escapeHtml(item.row)} 行提示词" title="删除提示词" ${editable ? "" : "disabled"}><i data-lucide="trash-2" aria-hidden="true"></i></button>
      </article>`;
    }).join("");
    window.lucide?.createIcons();
  ...
  if (els.batchPageSize) els.batchPageSize.value = String(state.batchPageSize);
  if (els.batchPageState) els.batchPageState.textContent = `第 ${state.batchPage} / ${totalPages} 页 · 当前 ${filteredEntries.length} / 共 ${state.batchPrompts.length} 条`;
  ...
  const selectable = batchSelectableEntries();
  ...
  if (els.submitBatchTasks) {
    els.submitBatchTasks.disabled = state.batchAutoStopRequested || (!state.batchAutoRunning && !state.batchSubmitting && (state.batchJobId || !selected.length));
    els.submitBatchTasks.textContent = state.batchAutoRunning || state.batchSubmitting
      ? (state.batchAutoStopRequested ? "正在停止" : "停止生成")
      : state.batchJobId ? "批次已提交" : "生成所选任务";
  }
  if (els.autoSubmitBatchTasks) {
    const configurable = !state.batchJobId && state.batchPrompts.some((item) => !batchItemIsCreated(item) && item.prompt.trim());
  ...
  const failedRows = state.batchPrompts.filter((item) => String(item.status || "") === "failed" && String(item.taskId || ""));
  if (els.retrySelectedBatchTasks) {
    els.retrySelectedBatchTasks.disabled = state.batchRetrying || !state.batchJobId || !selected.length;
    els.retrySelectedBatchTasks.textContent = state.batchRetrying ? "重试处理中" : `重试所选失败任务${selected.length ? ` (${selected.length})` : ""}`;
  }
  if (els.retryAllFailedBatchTasks) {
    els.retryAllFailedBatchTasks.disabled = state.batchRetrying || !state.batchJobId || !failedRows.length;
    els.retryAllFailedBatchTasks.textContent = `重试全部失败任务${failedRows.length ? ` (${failedRows.length})` : ""}`;
  }
...
function applyBatchSelectionCount() {
  const availableCount = batchSelectableEntries().length;
  ...
  state.batchPrompts.forEach((item) => {
    if (!batchSelectableEntries().includes(item)) return;
...
async function parseBatchSpreadsheet() {
  const file = state.batchSpreadsheet;
  if (!file) return toast("请先选择表格文件", "error");
  const form = new FormData();
  form.append("spreadsheet", file, file.name);
  setBusy(els.parseBatchSpreadsheet, true, "解析中");
...
async function retryFailedBatchTasks({ retryAll = false } = {}) {
  const jobId = String(state.batchJobId || "");
  if (!jobId || state.batchRetrying) return;
  const selected = batchSelectedEntries();
  const rowIndices = selected.map(({ item }) => Math.max(1, Number(item.batchRowIndex || 0))).filter(Boolean);
  const requested = retryAll
    ? state.batchPrompts.filter((item) => String(item.status || "") === "failed" && String(item.taskId || "")).length
    : rowIndices.length;
  if (!requested) return toast("请先筛选并选择失败任务", "error");
  const message = retryAll
    ? `确认重试全部 ${requested} 个失败任务吗？每个重试任务会按当前提交间隔依次入队。`
    : `确认重试所选的 ${requested} 个失败任务吗？每个重试任务会按当前提交间隔依次入队。`;
  if (!window.confirm(message)) return;
  state.batchRetrying = true;
  renderBatchPrompts();
  try {
    const data = await apiFetch(`/batch-prompts/jobs/${encodeURIComponent(jobId)}/retry`, {
      method: "POST",
      body: retryAll ? { retry_all: true } : { row_indices: rowIndices },
      timeout: 120000,
    });
    state.batchStatusFilter = "all";
    if (els.batchStatusFilter) els.batchStatusFilter.value = "all";
    ...
    state.batchJobCursorReady = false;
    state.batchPrompts = [];
    const active = applyPersistentBatchJob(data.job);
    toast(`已创建 ${Number(data.created || 0)} 个重试任务`);
    if (active && !state.batchAutoRunning) monitorPersistentBatchJob(data.job.id);
  } catch (error) {
    toast(`批量重试失败：${batchFriendlyError(error.message)}`, "error");
  } finally {
    state.batchRetrying = false;
    renderBatchPrompts();
  }
...
async function submitBatchTasks() {
  ...
  if (state.batchJobId) return toast("当前批次已提交；失败任务请使用批量重试", "error");
...
function bindEvents() {
  ...
  els.membershipDetailsButton?.addEventListener("click", () => {
    renderMembershipDetails();
    openSettingsModal(els.membershipDetailsModal, els.closeMembershipDetailsModal);
  });
  ...
  els.billingPriorityControl?.addEventListener("change", async (event) => {
    const input = event.target.closest('input[name="billingPriority"]');
    if (!input || portal !== "client") return;
    const previous = state.billingPriority;
    state.billingPriority = input.value === "points_first" ? "points_first" : "video_first";
    applyAccessScope();
    try {
      const data = await apiFetch("/auth/billing-priority", { method: "PATCH", body: { priority: state.billingPriority } });
      applyAccessScope(data);
      toast("扣费优先级已更新");
    } catch (error) {
      state.billingPriority = previous;
      applyAccessScope();
      toast(`保存失败：${error.message}`, "error");
    }
  });
  ...
  els.batchRowImageInput?.addEventListener("change", () => {
    ...
    if (item) {
      releaseBatchImageEntries(item.images);
      item.images = createBatchImageEntries(els.batchRowImageInput.files || []);
    }
    state.batchImageTargetIndex = -1;
  ...
  els.batchPageSize?.addEventListener("change", () => {
    const requested = Number(els.batchPageSize.value || 10);
    state.batchPageSize = [10, 30, 50].includes(requested) ? requested : 10;
    state.batchPage = 1;
    renderBatchPrompts();
  });
  els.batchStatusFilter?.addEventListener("change", () => {
    state.batchStatusFilter = ["all", "generating", "success", "failed"].includes(els.batchStatusFilter.value) ? els.batchStatusFilter.value : "all";
    state.batchPage = 1;
    renderBatchPrompts();
  });
  els.batchPrevPage?.addEventListener("click", () => {
    state.batchPage = Math.max(1, state.batchPage - 1);
    renderBatchPrompts();
  });
  els.batchNextPage?.addEventListener("click", () => {
    state.batchPage += 1;
    renderBatchPrompts();
  });
  ...
  els.selectAllBatchPrompts?.addEventListener("change", () => {
    ...
    state.batchPrompts.forEach((item) => {
      if (batchSelectableEntries().includes(item)) {
        item.selected = selected;
      }
    });
    renderBatchPrompts();
  });
  els.batchPromptList?.addEventListener("change", (event) => {
    const checkbox = event.target.closest("[data-batch-prompt-select]");
    if (!checkbox) return;
    const item = state.batchPrompts[Number(checkbox.dataset.batchPromptSelect)];
    if (item && batchSelectableEntries().includes(item)) {
  ...
  els.batchPromptList?.addEventListener("click", (event) => {
    ...
    if (deleteButton) {
      ...
      const item = state.batchPrompts[index];
      if (!item || state.batchSubmitting) return;
      releaseBatchImageEntries(item.images);
      state.batchPrompts.splice(index, 1);
  ...
  els.batchPromptList?.addEventListener("input", (event) => {
    ...
    if (els.submitBatchTasks) els.submitBatchTasks.disabled = state.batchJobId || state.batchAutoRunning || (!state.batchSubmitting && !selectedCount);
    scheduleBatchDraftSave();
  });
  els.submitBatchTasks?.addEventListener("click", submitBatchTasks);
  els.retrySelectedBatchTasks?.addEventListener("click", () => retryFailedBatchTasks());
  els.retryAllFailedBatchTasks?.addEventListener("click", () => retryFailedBatchTasks({ retryAll: true }));
  els.refreshBatchTasks?.addEventListener("click", () => refreshBatchTaskStatuses());
  els.autoSubmitBatchTasks?.addEventListener("click", () => {
    if (state.batchJobId || state.batchAutoRunning || state.batchSubmitting) return;

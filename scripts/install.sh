...
GITHUB_OWNER="${GITHUB_OWNER:-1055660108}"
...
install_system_dependencies_apt() {
  local packages="python3 python3-venv python3-pip curl ca-certificates unzip redis-server"
  local mirror
  backup_apt_sources
...
install_system_dependencies_rhel() {
  local installer="$1"
  $installer install -y python3 python3-pip curl ca-certificates unzip redis || return 1
...
install_system_dependencies() {
  ...
  command -v redis-server >/dev/null 2>&1 || die "redis-server 安装失败"
  systemctl enable --now redis-server 2>/dev/null || systemctl enable --now redis 2>/dev/null || die "Redis 启动失败"
...
Environment=DOLA_QUEUE_BACKEND=redis
Environment=DOLA_REDIS_URL=redis://127.0.0.1:6379/0
...
[Unit]
Description=dola_fetch_worker
After=network.target redis-server.service ${SERVICE_NAME}.service
...
EOF
  ...
  systemctl is-active --quiet "${SERVICE_NAME}-worker" || {
...
PY
  ...
  echo "重启 Worker：systemctl restart ${SERVICE_NAME}-worker"

...
class AdminUITests(unittest.TestCase):
    ...
    def test_spreadsheet_batch_submission_is_complete_and_editable(self) -> None:
        ...
        self.assertIn('lazy_assets: true', self.javascript)
        self.assertIn('/rows/${encodeURIComponent(rowIndex)}/assets', self.javascript)
        self.assertIn('await uploadRequestedBatchAssets(result.job)', self.javascript)
        self.assertIn('return "正在上传当前任务参考图"', self.javascript)
        ...
        self.assertNotIn("prepareBatchTaskImageAssets(sessionId, selected)", windowed_submit_body)

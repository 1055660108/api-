...
class AdminUITests(unittest.TestCase):
    ...
    def test_task_status_contract_uses_authoritative_terminal_state(self) -> None:
        ...
        self.assertIn('if (!terminal && /生成超过\\d+分钟', self.javascript)
    ...
    def test_client_ink_entry_flow_preserves_existing_auth_and_workspace(self) -> None:
        ...
        self.assertIn('/admin/assets/ink-bg.js?v=1.4.32', self.html)
    ...
    def test_spreadsheet_batch_submission_is_complete_and_editable(self) -> None:
        ...
        self.assertIn('for (let attempt = 0; attempt < 3; attempt += 1)', self.javascript)
        ...
        self.assertIn('while ((active.size || cursor < selected.length) && !state.batchAutoStopRequested && !stopped)', self.javascript)
        self.assertIn('active.size + outsideActive < concurrency', self.javascript)
        self.assertIn('outsideActive = Math.max(0, Number(state.userActiveTaskCount || 0) - active.size)', self.javascript)
        self.assertIn('批次排队 ${waiting} 条', self.javascript)
        ...
        self.assertIn("createBatchTask(entry, sessionId, ratio, referenceBundle)", windowed_submit_body)
        self.assertIn("active.size + outsideActive < concurrency", windowed_submit_body)

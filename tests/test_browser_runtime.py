...
class BrowserRuntimeTests(unittest.TestCase):
    ...
    def test_three_platforms_cleanup_before_closing_playwright(self) -> None:
        ...
        self.assertIn("proxy_config=proxy_config", doubao)
        self.assertNotIn("await bounded_cleanup(lease.release())", doubao)
        self.assertNotIn("await self.browser_pool.acquire_context(", doubao)
        self.assertNotIn('context_options["storage_state"] = storage_state', doubao)
        self.assertIn("await context.add_cookies(cookies)", doubao)
        self.assertIn("launch_persistent_context", doubao)
        self.assertIn("DOUBAO_PROFILES_DIR", doubao)
        self.assertIn("await bounded_cleanup(safe_close(context))", doubao)
        self.assertNotIn("BROWSER_INIT_SCRIPT", doubao)
        self.assertNotIn("BROWSER_EXTRA_HTTP_HEADERS", doubao)
        self.assertIn("DOUBAO_BROWSER_USER_AGENT", doubao)
        self.assertIn("X11; Linux x86_64", doubao)
        self.assertIn("await context.add_init_script(BROWSER_INIT_SCRIPT)", dola)
        self.assertIn("find_slider_page", doubao)
        self.assertIn("coordinate_solver_from_environment", doubao)
        self.assertIn("ancestor::*[contains(@class, 'drag-area')][1]", doubao)
        self.assertIn("duplicate_image_tile", doubao)
        self.assertIn("semantic captcha drag was not confirmed", doubao)
        self.assertIn("internal_registered_selection", doubao)
        self.assertIn("semantic captcha selected answers were not stable", doubao)
        self.assertIn("doubao_verification_drag_expected_tile_keys", doubao)
        self.assertNotIn("if selected_after > selected_before or dropped_after > dropped_before", doubao)
        self.assertIn("verification_recovery_delay_ms", doubao)
        self.assertIn("await page.evaluate(", doubao)
        self.assertIn("DOUBAO_SUBMIT_SCRIPT", doubao)
        self.assertIn('"doubao_result_mode": "interface_poll" if conversation_id else "browser_fallback"', doubao)
        self.assertIn('"keep_account_claimed": True', doubao)
        self.assertIn("submission_pacer=self._wait_for_doubao_submit_slot", (root / "worker.py").read_text(encoding="utf-8"))
        self.assertIn('platform="doubao"', (root / "worker.py").read_text(encoding="utf-8"))
        self.assertIn('candidate_platform in {"dola", "doubao"}', (root / "worker.py").read_text(encoding="utf-8"))
        self.assertIn("image_upload_slot=(", (root / "worker.py").read_text(encoding="utf-8"))
        ...
        self.assertIn("DOUBAO_PREHANDLE_ATTACHMENTS_SCRIPT", doubao)
        self.assertIn('await context.storage_state(path=str(self.state_path), indexed_db=True)', doubao)
        self.assertIn('self._set_phase("submitting_video_creation_request"', doubao)
        self.assertIn("await self._submit_via_video_creation_page_ui(page, image_count=image_count)", doubao)
        self.assertIn("submission_request_seen = asyncio.Event()", doubao)
        self.assertIn('await editor.press("Enter")', doubao)

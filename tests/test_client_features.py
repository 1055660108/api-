...
class ClientFeatureTests(unittest.TestCase):
    ...
    def test_membership_catalog_admin_crud_and_public_filtering(self) -> None:
        ...
        legacy_entry = next(item for item in user_data["users"].values() if item["id"] == user_id)
        legacy_entry.pop("membership_concurrency_mode", None)
        legacy_entry["memberships"][0]["concurrency"] = 2
        users.USERS_PATH.write_text(json.dumps(user_data, ensure_ascii=False), encoding="utf-8")
        temp_access.update_temp_token(token_id, concurrency=2)
        self.assertEqual(self.client.get("/auth/access-state", headers=headers).json()["token_concurrency"], 3)
    ...
    def test_membership_discount_reduces_paid_video_cost(self) -> None:
        ...
        no_discount = self.client.post("/admin/memberships", json={"name": "无减免套餐", "points_cost": 1, "duration_days": 30, "concurrency": 1, "task_discount_points": 0})
        self.assertEqual(no_discount.status_code, 201)
        self.assertEqual(no_discount.json()["package"]["task_discount_points"], 0)
        self.assertEqual(self.client.post(f"/memberships/{no_discount.json()['package']['id']}/purchase", headers=headers).status_code, 200)
        with patch.object(main, "active_task_count_for_owner", return_value=0), patch.object(main, "create_sem", asyncio.Semaphore(1)):
            free_task = self.client.post("/tasks", headers=headers, data={"prompt": "免费视频额度任务", "ratio": "9:16", "platform": "dola", "model": "Seedance 2.0", "task_type": "video"})
            full_price_task = self.client.post("/tasks", headers=headers, data={"prompt": "零减免任务", "ratio": "9:16", "platform": "dola", "model": "Seedance 2.0", "task_type": "video"})
        self.assertEqual(free_task.status_code, 200)
        self.assertEqual(full_price_task.status_code, 200)
        self.assertEqual(full_price_task.json()["billing"]["points_used"], 1)
        created = self.client.post("/admin/memberships", json={"name": "减免会员", "points_cost": 2, "duration_days": 30, "concurrency": 1, "bonus_free_uses": 0, "task_discount_points": 0.3})
        package_id = created.json()["package"]["id"]

...
class DoubaoAutomationTests(unittest.TestCase):
    ...
    def test_runtime_submission_uses_native_video_page_controls(self) -> None:
        ...
        self.assertIn('[role="button"]:visible, [tabindex="0"]:visible', ui_source)
        self.assertIn("Continue waiting for the visible acknowledgement after the click", ui_source)
        self.assertIn("video_creation_send_button_fallback_used", ui_source)
        self.assertNotIn('"video_creation_ui_error": "doubao video send button obstructed"', ui_source)

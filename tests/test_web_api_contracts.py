...
class WebAPIContractTests(unittest.TestCase):
    ...
    def test_runtime_submit_interval_is_admin_only_persistent_and_validated(self) -> None:
        ...
        self.assertEqual(current["dola_exit_submit_interval_seconds"], 8.0)
        self.assertEqual(current["task_retry_limit"], 2)
        self.assertEqual(current["batch_history_retention_days"], 30)
        updated = self.client.post("/config/runtime", json={"dola_submit_interval_seconds": 2.5, "dola_exit_submit_interval_seconds": 9.5, "task_retry_limit": 5, "batch_history_retention_days": 14})
        self.assertEqual(updated.status_code, 200)
        self.assertEqual(updated.json()["dola_submit_interval_seconds"], 2.5)
        self.assertEqual(updated.json()["dola_exit_submit_interval_seconds"], 9.5)
        ...
        self.assertEqual(config.load_settings().dola_exit_submit_interval_seconds, 9.5)
        ...
        for invalid in (2.9, 30.1, "invalid"):
            self.assertEqual(self.client.post("/config/runtime", json={"dola_exit_submit_interval_seconds": invalid}).status_code, 400)
    ...
    def test_account_user_and_configuration_responses_keep_web_contracts(self) -> None:
        ...
        self.assertEqual((workers["browser_pool_processes"], workers["browser_contexts_per_process"], workers["submission_concurrency"]), (12, 4, 48))
    ...
    def test_global_worker_configuration_keeps_remote_generation_unlimited(self) -> None:
        ...
        self.assertEqual(payload["browser_workers"], 48)
        self.assertEqual(payload["max_effective_workers"], 48)
        self.assertEqual(payload["capacity_limit"], 48)
        self.assertEqual(payload["effective_browser_workers"], 48)
        self.assertEqual(payload["remote_generation_limit"], 0)
        self.assertEqual((payload["browser_pool_processes"], payload["browser_contexts_per_process"], payload["submission_concurrency"]), (12, 4, 48))
        ...
        self.assertEqual(config.load_settings().browser_workers, 48)
        ...
        self.assertEqual(config.load_settings().max_effective_workers, 48)
